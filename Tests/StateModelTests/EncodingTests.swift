...
struct EncodingTests {
    ...
    func testStateCommand() throws {
        ...
        let command = CommandRequest(path: path, arguments: arguments)
    ...
    func testPropertyChange() throws {
        let change = PropertyUpdate(id: 123, date: Date(), data: .init(repeating: 42, count: 13))

        let data = try JSONEncoder().encode(change)
        let decoded = try JSONDecoder().decode(PropertyUpdate.self, from: data)
